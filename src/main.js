...
function updateCollege(profileData) {
    const col = document.getElementById('profile.education.college')
    col.innerHTML = profileData.education.college.map((data) => 
        `<li>${data.description}</li>
        <li>${data.period}</li>`).join('')
}

function updateCertificates(profileData){
    const certificates = document.getElementById('profile.certificates')
    certificates.innerHTML = profileData.certificates?.map(cert => {
        return `
            <li>
                <h3 ${cert.site ? 'class="certificates"' : ''}>${cert.name}</h3>
